...
- "Aynı değerle PATCH" (değişiklik yok): `name`/`active` yalnız GERÇEKTEN mevcut değerden farklıysa değişiklik sayılır (businesses.version artar, audit yazılır); hiçbir alan gerçek değişiklik getirmiyorsa (hepsi aynı) 422 VALIDATION_FAILED `{ change: "Değişiklik yok." }` döner, sürüm artmaz, audit yazılmaz. Sahip atama/adı düzeltmesi bu "aynı değer" muafiyetine TABİ DEĞİLDİR (yapısal işlemlerdir, kendi hata sınıflarıyla — "zaten bir sahibi var" / "sahibi yok" — ayrıca korunur).
