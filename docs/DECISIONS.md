...
- **Açık engel:** `load:run` hazırlıkta ve koşu boyunca `GET /api/v1/health/ready` çağırır; Caddy sağlık uçlarını dışarıya 404 ile kapatır (`deploy/caddy/Caddyfile` `@health`). Hedefin dışından koşu bu yüzden hazırlıkta durur; kodda giderilene dek hedefte hiçbir senaryo koşulamaz. Sağlık uçlarını dışarı açmak veya üreticiyi hedefe taşımak çözüm sayılmaz.
