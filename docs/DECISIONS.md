...
- Parola alanları `request_hash`'e HİÇ dahil edilmez (ARCH §6 — kısa parolanın SHA-256'sı çevrimdışı kırılabilir); bunun bedeli, aynı requestId+FARKLI parolalarla yapılan bir tekrar gönderimin hash karşılaştırmasıyla YAKALANAMAMASIDIR. Bunu kapatmak için `createVehicle`, bilinen bir makbuzla karşılaşınca (transaction'a girmeden, salt okunur bir ön bakışla) gönderilen iki parolayı da saklanan Argon2 özetlerine karşı (hash kuyruğu üzerinden) DOĞRULAR; eşleşmezse 201 yerine 409 REQUEST_ID_REUSED döner. Bu ön bakış ile asıl yazma transaction'ı arasındaki (aynı requestId'nin GERÇEKTEN eşzamanlı iki isteğiyle sınırlı) dar yarış penceresinde parola yeniden doğrulaması ATLANIR — transaction içi Argon2 çağrısı better-sqlite3'ün senkron transaction kısıtını ihlal ederdi; birincil savunma bu ön bakıştır, kalan pencere yalnız çift-insert'ü (idempotency PK'sı zaten) önler.
