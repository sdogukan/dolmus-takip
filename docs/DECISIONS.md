...
- **CI:** dört ayrı adım (`ci-steps.json`, `ci.yml`, `release.yml`) tek `quality-gate` adımı oldu, `test:release`'ten önce; `timeout-minutes: 30` değişmedi. Kapı iş başına bir kez koşar; `test:release` kaydı klona kopyalar, `release:build` kaydı kullanır.
- **Meta-test:** `tests/release/release-build.test.ts` 4 testin hepsini korur. Test 3'ün ikinci commit'i `--allow-empty` (farklı SHA, aynı ağaç); Test 4'ün başarısız testli commit'i ağacı değiştirir, kaydı bulamaz, kapı `test:unit`'te durur ve kayıt yazılmaz. Ölçüm (DIJJI konteyneri, kayıt kopyalanamadığı için kapı klonda bir kez tam koştu): dosya 786 s, bunun ~629 s'si `beforeAll`, testler 57,8 / 1,7 / 57,1 / 40,8 s. Zaman aşımları: `beforeAll` 20 dk, testler 5 dk. ubuntu-24.04 runner'ında 30 dk iş bütçesine sığdığı ölçülmedi.
