...
- **Otomatik yayın (2026-09-26, kullanıcı kararı — "yayın elle, SSH ile manuel tetiklenir" kuralının yerini alır):** `main`'e gelen push'un CI koşusu yeşil bitince `.github/workflows/deploy.yml` o koşunun `release-<sha>` paketini (yeniden derleme yok) SSH ile sunucuya gönderir; sunucuda `deploy` kullanıcısının anahtarı yalnız `/usr/local/sbin/dolmus-deploy-receive`'i çalıştırabilir (authorized_keys `restrict,command=`, sudoers tek komut). Alıcı SERVER-SETUP §4 adımlarını birebir uygular (hash → yeni release dizini → `release-apply deploy` → dış `https://<alan>/giris` 200 → `cleanup`; `mark-verified` bilerek yok: doğrulanmamış yayında önceki release ve yayın öncesi DB kopyası korunur, RELEASE §7 geri dönüşü bir sonraki yayına kadar açık kalır) ve önceki yayının commit'ini döner; iş akışı önceki yayından yeniye kadar main'in her birinci-ebeveyn commit'ini Dijji deploy webhook'una bildirir (Dijji merge commit'ini birebir eşler). Yalnız main'in en son commit'i kurulur; CI kırmızıysa hiçbir şey kurulmaz. SSH (22) GitHub runner'ları için dünyaya açıldı; parola girişi kapalı. Gerekçe: Dijji PR'ı merge edip deploy bekler; elle deploy her task'ta insan beklemesi yaratıyordu. Açık risk: tek aşama olduğu için Dijji E2E'si canlıya çıktıktan sonra koşar; gerçek kullanıcılardan önce staging aşaması eklenecek.
...
- CI (`scripts/ci-steps.json`, `ci.yml`, `release.yml`) `test:release`'i `test:integration`'dan hemen sonra ayrı adım olarak koşar; kapsam değişmedi, hiçbir test atlanmadı. Bu taşıma dosyanın süresini kısaltmadı: her tam `release:build` kapıyı yine koşuyordu (build testi başına ölçülen 642 s ve 644 s). Süre sorunu aşağıdaki "Kalite kapısı kaydı" kararıyla çözüldü.
...
## Kalite kapısı kaydı (2026-09-25)
- **Karar (onaylı seçenek 2):** `release:build`, kapının aynı içerik için geçtiğini kanıtlayan geçerli bir kayıt varsa kapıyı yeniden çalıştırmaz; yoksa `typecheck` → `lint` → `test:unit` → `test:integration`'ı eskisi gibi tam çalıştırır. Kural: çağıran kim olursa olsun, kapının bu commit'in içeriği için geçtiği kanıtlanmadan arşiv üretilmez.
- **Anahtar commit SHA'sı değil tree hash'i** (`git rev-parse HEAD^{tree}`): kapının sonucu içeriğe (lockfile dahil) bağlıdır, commit meta verisine değil. Kayıt izlenebilirlik için commit SHA'sını da taşır.
- **Konum ve biçim:** gitignore'lu `/.quality-gate/<tree>.json`; alanlar `version`, `tree`, `commit`, `node_version`, `steps`, `passed_at`. Geçici dosya + `rename` ile yazılır. Gitignore'da olmasaydı `git status --porcelain` onu görür ve `release:build` her ağacı kirli sayardı.
- **Geçerlilik:** tree, adım listesi (`scripts/lib/quality-gate.ts` `QUALITY_GATE_STEPS`, tek tanım) ve `process.version` o an git'ten ve çalışma zamanından hesaplanıp kayıtla birebir karşılaştırılır; kayıttaki veya dosya adındaki değerlere güvenilmez. Yok, bozuk ya da tutmayan kayıt = tam kapı, hiçbir zaman çökme değil.
- **Yazan:** yalnız dört adımı exit 0 ile bitiren, çalışma ağacını kapı başında ve sonunda temiz, HEAD'i sabit gören koşu (`npm run quality-gate` ya da `release:build`'in kendi tam kapısı). Başarısız ya da yarım kapı kayıt yazmaz; kirli ağaçta adımlar koşar, kayıt yazılmaz. Kirli ağaç reddi `release:build`'de kayıt mantığından önce gelir; `--allow-dirty` benzeri bayrak yok.
- **CI:** dört ayrı adım (`ci-steps.json`, `ci.yml`, `release.yml`) tek `quality-gate` adımı oldu, `test:release`'ten önce; `timeout-minutes: 30` değişmedi. Kapı iş başına bir kez koşar ve `release:build` onun kaydını kullanır. `test:release` kök kayda bakmaz; klona kendi hazır kaydını yazar (Meta-test).
- **Meta-test:** `tests/release/release-build.test.ts` 4 testin hepsini korur. Test 3'ün ikinci commit'i `--allow-empty` (farklı SHA, aynı ağaç); Test 4'ün başarısız testli commit'i ağacı değiştirir, kaydı bulamaz, kapı `test:unit`'te durur ve kayıt yazılmaz. `beforeAll` klonun kendi ağacı için `writeRecord` ile hazır bir kayıt (fixture) yazar; kalite kapısını klonda koşmaz, proje kökündeki kaydı okumaz ya da kopyalamaz. Kapının doğruluğu `tests/unit/quality-gate.test.ts` ve Test 4'le sınanır; bu dosya boru hattını sınar. Önceki yaklaşım (kök kaydı klona kopyala, yoksa kapıyı klonda koş) DIJJI konteynerinde kök `CLAUDE.md` `skip-worktree` ile değiştiği için klon ağacını kayıttan ayırdı ve kapıyı klonda tam koşturdu (dosya 786 s, bunun ~629 s'si `beforeAll`); bunu kapatan git-view kopya yardımcısı da fixture ile kaldırıldı. Ölçüm (DIJJI konteyneri, fixture ile): `beforeAll` adımları toplam ~2,2 s, testler 57,7 / 1,7 / 58,9 / 42,5 s. Zaman aşımları: `beforeAll` 2 dk, testler 5 dk. ubuntu-24.04 runner'ında 30 dk iş bütçesine sığdığı ölçülmedi.
- **Sınır:** kayıt bir kaza korumasıdır, kurcalamaya karşı değil — checkout'a yazabilen biri kaydı da taklit edebilir, `release-build.ts`'i düzenleyebileceği gibi. Dayanacak anahtar olmadığı için imza eklenmedi. `test:integration`'ı kısaltmak bu kararın kapsamı dışında.
