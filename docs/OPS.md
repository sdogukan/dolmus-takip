...
**Durum:** Koddan önce hazırlanmış işletim planı; sunucu, izleme, alarm veya yedek kurulmadı. Gerçek olay/test kanıtı yoktur.  
...
| Dış kontrol ve ekip uyarı kanalı | Makineden bağımsız kontrolün yeri, gerçek kanal ve deneme bildiriminin ulaştığı kişi kaydedilir |
...
| Yerel canlılık | `/api/v1/health/live`; 30 saniyede bir, 3 saniye timeout, 3 ardışık hata; başlangıç toleransı 90 saniye | Bakım/kilit/bütçe uygunsa sınırlı düzeltici restart; donma yolunu uygula |
...
| Disk | Başlangıç önerisi: %80 uyarı, %90 kritik | Büyümeyi ölç; koruma kurallarıyla güvenli temizlik planla. Yazma hatası/tutarlılık riski varsa bakıma al |
| Log hacmi | Architecture örnek başlangıç sınırı toplam 200 MB; uygulamada journald kapsamı doğrulanır | Sınırlı döndürmeyi doğrula; mali kayıt/revizyonları log temizliğiyle silme |
| RAM, CPU/burst, event-loop | Yük testi tabanı, OOM ve sürekli baskı; sayısal alarm eşikleri ölçümle kesinleşir | Süreç/sorgu nedenini araştır; kaynak veya DB değişikliğini gerekçesiyle ayrıca değerlendir |
| SQLite bekleme ve WAL | Başlangıç `busy_timeout=2000ms`; tekrarlayan kilit hatası veya sürekli WAL büyümesi | Uzun transaction/okumayı araştır; sınırsız tekrar veya WAL silme uygulama |
...
Loglar zaman, request_id, sürüm, işlem türü ve gerekli hata kodunu taşır; şifre/token/oturum değeri veya gereksiz kişisel mali veri içermez. İşlem geçmişi DB'de korunur ve teknik loglardan ayrıdır. Olay kanıtında UTC; kullanıcıyla paylaşımda açıkça Europe/Istanbul kullanılır.
...
**Müdahale:** Başlangıç toleransı/bakımı kontrol et. Bağımsız görev, sınırlar uygunsa en çok iki düzeltici restart dener; herhangi bir kurtarma sınırında kilit kalıcılaşır. Log, event-loop ve kaynak baskısını incele; kilidi veya systemd sayacını otomatik sıfırlama.  
**Eskalasyon / dönüş:** Sorumlu nedeni gidermeden kilit açılmaz; reboot çözüm diye tekrarlanmaz. Kontrollü açılış ve canlılık/hazırlık doğrulanır; yeni hata varsa tekrar müdahale kaydı tutulur.
...
**Müdahale:** Dış kontrol/DNS/TLS ile AWS makine durumunu ayır; yalnız uygulama restart'ı varsayma. Son snapshot/manifest ilişkisini kontrol et; sınırsız reboot veya otomatik eski DB restore'u başlatma.  
