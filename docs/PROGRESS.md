...
- [ ] T6.6 (S6.6) Yük/veri bütünlüğü kabulü ve pilot rehberi — durum: kit hazır (`runtime_metrics` satırı, `load:seed`, `load:run`, `integrity:check`, süreç öldürme bütünlük testi) ve rehberler yazıldı (QA-PLAN §2 izlenebilirlik tablosu ve §3 yük prosedürü, RELEASE §4 kapı, OPS §8, SERVER-SETUP §5.1); hedef sunucuda HİÇBİR ölçüm yapılmadı, kabul tablosunun tüm satırları çalıştırılmadı/açık, pilot için hazır değil. `load:run` hazırlık ve koşu içi sondası Caddy üzerinden `GET /giris`tir (sağlık uçları yalnız localhost; DB hazırlığı `dolmus-health` `event=probe` / `ready_ms`'ten okunur; bkz. DECISIONS "T6.6 ... kararları")
