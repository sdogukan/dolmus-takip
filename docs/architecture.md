...
- Report module (src/server/usecases/reports, partly implemented) — vehicle period report (readVehiclePeriodReportForScope, behind GET /api/v1/reports/vehicles: one SELECT over the business + vehicle scoped work_entries in [startDate, nextStartDate) with COUNT, COUNT(DISTINCT work_date) and SUM aggregates, cent totals returned as exact decimal-integer text, the received total taken only from the current version's cash confirmation of driver-kind entries; writes nothing); period resolution (week starting Monday, month, year) in src/lib/report-period.ts; person and day-by-day reports planned (M5)
