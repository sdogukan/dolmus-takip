...
| GET | /api/v1/work-entries/:id | Read one entry (implemented T3.5): same shape as the POST 201 workEntry, with confirmation { receivedCents, confirmedAt, entryVersion } for the current version only (null when unconfirmed; no actor or session id); an unknown id, another vehicle's or business's entry and, for drivers, an owner-kind entry or one whose person is no longer selectable all get the same 404 WORK_ENTRY_NOT_FOUND | Driver (K1), owner or staff with X-Target-Vehicle (work_entry.read) |
