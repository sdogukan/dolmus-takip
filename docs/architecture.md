...
- Caddy (deploy/caddy/Caddyfile, prepared, not tried on a real server — ISSUE-29) — TLS with automatic certificate for {$DOLMUS_DOMAIN}, HTTP→HTTPS redirect, request body limit 1 MB, /api/v1/health and /api/v1/health/* answered 404 to the outside, no file serving, no trusted_proxies (client X-Forwarded-For is overwritten); exposes 80/443, proxies only to 127.0.0.1:3000; maintenance response not in the Caddyfile yet
