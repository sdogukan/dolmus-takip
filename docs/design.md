...
| # | Page | URL |
| --- | --- | --- |
| 1 | Şoför — günlük kayıt formu (implemented T3.1–T3.4, shared WorkEntryForm in driver mode: fixed plate, date, person picker, start/end time with a 'Bitiş ertesi gün' box and live duration, gross and fuel, one optional other expense + note, live read-only share and hand-over summary, 'Kaydet' saves the entry (T3.4) and replaces the form with 'Kaydedildi', '<name> · <date> · <duration>', 'Teslim edilecek tutar' and 'Henüz doğrulanmadı'; an unknown result locks the fields with 'Kaydı tekrar dene') | /sofor |
| 2 | Şoför — kayıt sonucu / teslim durumu (planned) | /sofor/kayitlar/:id |
| 3 | Sahip — özet (placeholder implemented with '+ Çalışma kaydı gir' and 'Şoförlerim' links; content M5) | /sahip |
| 4 | Sahip — çalışma kaydı (implemented T3.3–T3.4: plate header, '← Özet', 'Kayıt türü' choice 'Kendim çalıştım' / 'Şoför adına' with nothing preselected; owner kind shows 'Çalışan: <owner> · Mal sahibi', 'Şoför payı' 0,00 TL, 'Giderlerden sonra kalan' and 'Kendi çalışmanda şoför payı ayrılmaz.'; driver kind shows the active-driver picker 'Şoförü seç' and 'Şoför payı (%20)' / 'Teslim edilecek tutar'; same date, time and amount fields as the driver form; 'Kaydet' saves the entry (T3.4); owner kind ends with 'Giderlerden sonra kalan' and 'Onay gerekmiyor', driver kind with 'Teslim edilecek tutar' and 'Henüz doğrulanmadı') | /sahip/kayit/yeni |
...
_Conflict between the approved wireframe (value kept unchanged) and the code written for this task (T3.4). After a 201, WorkEntryForm replaces the form in place (on /sofor, /sahip/kayit/yeni and /yonetim/araclar/:id/kayit/yeni) with a polite live region: 'Kaydedildi'; WORK_ENTRY_MESSAGES.savedPerson '<name> · <date> · <duration>'; 'Teslim edilecek tutar' (savedRemainder) or, for owner kind, 'Giderlerden sonra kalan' (savedOwnerRemainder) with the server's remainder; 'Henüz doğrulanmadı' for status pending or 'Onay gerekmiyor' for not_required; and a 'Başka bir çalışma kaydı gir' button that returns to an empty form. Not rendered, but in the approved wireframe: the plate on the result line, the '08:00–17:30' time range, the 'Mal sahibi parayı aldığında burada görebileceksin.' hint and the 'Yenile' button. The result/delivery page /sofor/kayitlar/:id is still planned (design Admin pages row 2). Split the wireframe into the implemented post-save confirmation and the planned result page, or keep it and extend the confirmation later?_
