...
| # | Page | URL |
| --- | --- | --- |
| 1 | Şoför — günlük kayıt formu (implemented T3.1–T3.2 without saving, shared WorkEntryForm in driver mode: fixed plate, date, person picker, start/end time with a 'Bitiş ertesi gün' box and live duration, gross and fuel, one optional other expense + note, live read-only share and hand-over summary, 'Kontrol et'; saving M3) | /sofor |
...
| 4 | Sahip — çalışma kaydı (implemented T3.3 without saving: plate header, '← Özet', 'Kayıt türü' choice 'Kendim çalıştım' / 'Şoför adına' with nothing preselected; owner kind shows 'Çalışan: <owner> · Mal sahibi', 'Şoför payı' 0,00 TL, 'Giderlerden sonra kalan' and 'Kendi çalışmanda şoför payı ayrılmaz.'; driver kind shows the active-driver picker 'Şoförü seç' and 'Şoför payı (%20)' / 'Teslim edilecek tutar'; same date, time and amount fields as the driver form; 'Kontrol et'; saving M3) | /sahip/kayit/yeni |
...
| 14 | Ekip — sahip adına çalışma kaydı (implemented T3.3 without saving: team header, pinned support target header, '← Destek'; 'Sahip çalıştı' / 'Şoför adına' with nothing preselected; driver list read with the X-Target-Vehicle header; inactive vehicle or business → notice and a locked form; saving M3) | /yonetim/araclar/:id/kayit/yeni |
...
_Conflict between the approved wireframe (value kept unchanged) and the code written for T3.1, still open after T3.2. T3.2 (this task) added, below the time fields, 'Hasılat (TL)', 'Mazot (TL)', a '+ Masraf ekle' toggle opening 'Diğer masraf (TL)' + 'Açıklama' with a 'Masrafı kaldır' button, and a read-only summary 'Şoför payın (%20)' / 'Teslim edilecek tutar' — the same order and labels as the approved wireframe, so that part no longer conflicts (the summary shows '—' until the amounts are valid). Remaining T3.1 differences: the approved layout orders 'Adın' (person) before 'Çalışma tarihi' (date) and shows no overnight control. WorkEntryForm renders, under the plate header and 'Günlük kayıt' heading: 'Çalışılan gün' (native date input prefilled with the server-computed Istanbul date, the formatted date '14 Eylül 2026' shown below it), 'Kim çalıştı?' (select starting at 'Adını seç'), 'Başlangıç saati', 'Bitiş saati' with a 'Bitiş ertesi gün' checkbox, live 'Bitiş: <date> <time>' when overnight and 'Süre: 9 saat 30 dakika'. Start and end times are stacked, not side by side. 'Kaydet' is not implemented yet (saving is T3.4); the single button is still 'Kontrol et'. Update the upper part of the wireframe to the implemented order and labels, or keep it and change the code later?_
...
_Conflict between the approved notes (value kept unchanged) and the code written for this task (T3.1). (1) Empty driver list text: approved 'Bu araç için şoför eklenmemiş. Mal sahibinden adını eklemesini iste.'; WORK_ENTRY_MESSAGES.personEmpty is 'Bu araçta seçilebilir şoför yok. Araç sahibinden şoför eklemesini iste.', shown only on 200 + drivers: [] (never on network, 401/403 or 5xx errors), with the picker disabled. (2) Overnight: approved 'overnight shows explicit end date ("Ertesi gün bitti")'; the code asks for an explicit 'Bitiş ertesi gün' checkbox and shows 'Bitiş: <date> <time>'; a same-day end before start is refused ('Bitiş saati başlangıçtan önce...') and equal same-day times are refused, never read as 24 h (evaluateWorkTime in src/lib/work-time.ts; entered times are always Europe/Istanbul, the device time zone is never used). (3) States not in the approved notes: list loading ('Şoförler yükleniyor…'), list error with 'Tekrar dene' (not after 401), submit re-reads GET /api/v1/drivers and a person no longer selectable clears the choice with 'Bu kişi artık bu araçta seçilemiyor. Liste yenilendi; adını yeniden seç.' while other fields are kept; overlapping list requests are sequenced so a stale response cannot overwrite a newer list. (4) T3.2 (this task) implements the approved Money line: gross and fuel required with explicit 0 allowed (empty → 'Tutarı gir. Yoksa 0 yaz.' under the field), text inputs with inputMode decimal, the summary shows '—' while any amount is empty or invalid and never keeps a stale total, a single other expense + optional note, and a negative hand-over shown as '-1.000,00 TL' with 'Giderler hasılatı ve payı aşıyor; teslim edilecek tutar eksi görünür. Tutarları kontrol et.' Not in the approved notes: removing a filled expense section asks 'Masraf kaldırılsın mı?' (Kaldır / Vazgeç) while an empty one closes at once; a note with an empty amount is an amount error, not 0; the note is limited to 200 characters; amounts are read as Turkish text only — '.' only as a thousands separator before 3-digit groups, ',' as the decimal separator, more than 2 decimal digits refused ('1.5' → an explanatory thousands-separator error, never read as 1,5); amounts beyond the safe integer limit → 'Tutar çok büyük.' or 'Yakıt, diğer gider ve pay toplamı çok büyük.' (parseTlAmount in src/lib/money.ts). The draft/request_id persistence and saving states are not implemented yet. Update the notes to the implemented texts and states, or keep them and change the code later?_
