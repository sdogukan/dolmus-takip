...
| # | Page | URL |
| --- | --- | --- |
| 1 | Şoför — günlük kayıt formu (implemented T3.1–T3.4, shared WorkEntryForm in driver mode: fixed plate, date, person picker, start/end time with a 'Bitiş ertesi gün' box and live duration, gross and fuel, one optional other expense + note, live read-only share and hand-over summary, 'Kaydet' saves the entry (T3.4) and replaces the form with 'Kaydedildi', '<name> · <date> · <duration>', 'Teslim edilecek tutar' and 'Henüz doğrulanmadı'; an unknown result locks the fields with 'Kaydı tekrar dene'; the saved result links 'Kaydı aç' to /sofor/kayitlar/:id; a 'Kayıtlarım' link sits below the form) | /sofor |
| 2 | Şoför — Kayıtlarım (implemented T3.6: 'Kimin kayıtları?' picker of selectable drivers, then that person's visible entries newest day first with day, time, duration, hand-over and status, 'Aç' and 'Daha fazla göster'; loading, error and empty states kept apart) | /sofor/kayitlar |
| 3 | Şoför — kayıt detayı ve düzenleme (implemented T3.6: 'Kayıt detayı' with the server's current values, version and status; the edit form opens only for a pending entry of today, otherwise read-only with the edit-window note; a driver-kind entry may switch to another selectable driver; the kind is shown, never editable; delivery status after cash confirmation planned M4) | /sofor/kayitlar/:id |
...
| 6 | Sahip — kayıt detayı ve düzenleme (implemented T3.6 for unconfirmed entries: '← Özet', 'Kayıt detayı', current server values incl. version and status, 'Kaydı düzenle' form with 'Değişiklikleri kaydet'; confirmed entries read-only; cash confirmation and correct-and-confirm planned M4) | /sahip/kayitlar/:id |
...
| 16 | Ekip — kayıt detayı ve düzenleme (implemented T3.6: team header, pinned support target header, '← Destek' back link; the same detail and edit form as the owner, sent with X-Target-Vehicle; inactive vehicle or business → notice and no editing; 'Hedefi değiştir' also clears this vehicle's edit drafts) | /yonetim/araclar/:id/kayitlar/:entryId |
...
_Conflict between the approved notes (value kept unchanged) and the code written for T3.1–T3.4. T3.4 (this task) implements saving: 'Kaydediliyor…' while sending; 'Kaydedildi' only after a 201, which also clears the draft; fields + requestId persisted in localStorage through useStoredDraft for 24 h and cleared on logout (F6); a missing session → COMMON_SCREEN_MESSAGES.sessionEnded. New T3.4 differences: (a) Unknown result — approved '"Kaydın sonucu kontrol ediliyor." — same request re-checked, form frozen'; the code freezes the request body into the draft before the fetch and, on a network error, unreadable body, 5xx or malformed 201, locks the fields with WORK_ENTRY_MESSAGES.unknownResult 'Kaydın gönderilip gönderilmediği bilinmiyor. Alanlar kilitlendi; tekrar dene, aynı kayıt yalnız bir kez oluşur.' and the button 'Kaydı tekrar dene'; there is no automatic re-check — the user's retry (also after a page reload) re-sends the frozen body byte for byte with the same requestId. (b) Known not-sent — approved 'Bağlantı yok. Henüz kaydedilmedi.' as the known-not-sent state; in the code a failed POST is always treated as unknown, and connectionFailed is shown only when the pre-send driver-list re-read fails or for an unmapped error code. (c) Not in the approved notes: 409 → 'Bu kayıt başka bir denemeyle çakıştı. Bilgileri kontrol edip yeniden kaydet.'; after an attempt that may have arrived, 401/403/404 keep the form locked with the same requestId and body, and only 422 and 409 REQUEST_ID_REUSED release it with a new requestId; a 422 on the person clears the choice and reloads the list; the server's own error message is never shown. Earlier differences still open: (1) empty driver list text — approved 'Bu araç için şoför eklenmemiş. Mal sahibinden adını eklemesini iste.', code WORK_ENTRY_MESSAGES.personEmpty 'Bu araçta seçilebilir şoför yok. Araç sahibinden şoför eklemesini iste.'; (2) overnight — approved 'Ertesi gün bitti' end date, code an explicit 'Bitiş ertesi gün' checkbox with 'Bitiş: <date> <time>' (evaluateWorkTime in src/lib/work-time.ts); (3) list loading, list error with 'Tekrar dene' and 'Bu kişi artık bu araçta seçilemiyor. Liste yenilendi; adını yeniden seç.' states; (4) T3.2 additions — removing a filled expense asks 'Masraf kaldırılsın mı?', a note with an empty amount is an amount error, the note is limited to 200 characters, Turkish amount parsing rules and too-large texts (parseTlAmount in src/lib/money.ts). Update the notes to the implemented texts and states, or keep them and change the code later?_
...
_Conflict between the approved wireframe (value kept unchanged) and the code written for T3.4 and this task (T3.6). Post-save result (T3.4, unchanged): WorkEntryForm replaces the form in place with 'Kaydedildi', '<name> · <date> · <duration>', 'Teslim edilecek tutar' (owner kind: 'Giderlerden sonra kalan'), 'Henüz doğrulanmadı' or 'Onay gerekmiyor' and 'Başka bir çalışma kaydı gir'. T3.6 adds a 'Kaydı aç' link (workEntryDetailHref) and implements /sofor/kayitlar/:id — previously the planned result/delivery page — as a detail + edit page: WorkEntryEditForm in driver mode with the h1 'Kayıt detayı', the 'Güncel kayıt' block (kind, person, day, time, duration, amounts, share, hand-over, 'Sürüm N', status) and 'Kaydı düzenle' only for a pending entry of today (otherwise WORK_ENTRY_MESSAGES.driverEditWindow 'Şoför olarak yalnız bugünün henüz doğrulanmamış kaydını düzenleyebilirsin.'). Still rendered nowhere, but in the approved wireframe: the plate on the result line, the '08:00–17:30' range on the post-save result, the 'Mal sahibi parayı aldığında burada görebileceksin.' hint and the 'Yenile' button. Update the wireframe to the post-save result + detail page, or keep it and extend the code later?_
...
_Conflict between the approved notes (value kept unchanged) and the code written for this task (T3.6). Approved: 'No "Kayıtlarım" privacy promise' (DESIGN §2.3: 'Kayıtlarım adıyla bireysel gizlilik güvencesi verilmez.'), confirmed state 'Teslim doğrulandı' + received amount + confirmation time, refresh on open and via 'Yenile'. Code: WORK_ENTRY_MESSAGES.listTitle and listLink are both 'Kayıtlarım' — /sofor links 'Kayıtlarım' below the daily form and /sofor/kayitlar carries the page title 'Kayıtlarım — Dolmuş Takip' and the heading 'Kayıtlarım', followed by the 'Kimin kayıtları?' person picker; DriverEntriesList and WorkEntryEditForm label a confirmed entry WORK_ENTRY_MESSAGES.statusConfirmed 'Onaylandı'; neither has a 'Yenile' control (the list loads on open and on person change; the detail re-reads only after a 409). Consistent with the notes: pending shows 'Henüz doğrulanmadı'; no 'teslim ettim' action is asked of the driver; the driver sees only entries of the selected person on the same vehicle (driver kind, person still selectable) and edits unconfirmed entries only on the work day (K1). Rename the list and use 'Teslim doğrulandı' in the code, or accept the implemented texts and drop the rule from the notes?_
...
_Conflict between the approved wireframe (value kept unchanged) and the code written for this task (T3.6). /sahip/kayitlar/:id and /yonetim/araclar/:id/kayitlar/:entryId render WorkEntryEditForm in owner / staff mode: back link WORK_ENTRY_MESSAGES.backToOwner '← Özet' (staff: '← ' + the support page title, under the pinned support target header), h1 'Kayıt detayı', a 'Güncel kayıt' list of rows (Kayıt türü 'Mal sahibi çalıştı' / 'Şoför çalıştı', Çalışan, Çalışılan gün, Saat 'HH:MM – HH:MM' plus 'Bitiş: <date> <time>' when overnight, Süre, Hasılat, Mazot, Diğer masraf with the note in brackets, Şoför payı, 'Teslim edilecek tutar' or 'Giderlerden sonra kalan', 'Sürüm N', status), then h2 'Kaydı düzenle' with the daily form fields and 'Değişiklikleri kaydet'. Differences from the approved wireframe: '< Özete dön'; a '<person> · <plate>' + '<date> · <time range>' header instead of rows; 'Beklenen teslim' instead of 'Teslim edilecek tutar'; 'Kaydı düzenle' as a link below the confirmation instead of an inline form; no version line in the approved layout. The received-amount field, 'Parayı aldım, tutar doğru' and the post-confirmation 'Düzelt ve onayla' state are not implemented yet (planned M4), so their absence is not a conflict. Add the implemented unconfirmed-edit state to the wireframe, or keep it and change the code when confirmation lands?_
