...
| 6 | Sahip — kayıt detayı, teslim onayı ve düzenleme (implemented T3.6 for unconfirmed editing and T4.2 for the first cash confirmation: '← Özet', 'Kayıt detayı', then a summary '<person> · <plate>', '<date> · <time range>', status and amount rows ending with 'Beklenen teslim' (driver kind) or 'Giderlerden sonra kalan' (owner kind), no version line; for a pending driver-kind entry 'Aldığım tutar (TL)' prefilled with the expected amount (not a confirmation), 'Beklenenden <amount> az./fazla.' and 'Parayı aldım, tutar doğru'; after confirmation 'Teslim doğrulandı', 'Alınan tutar' and 'Doğrulama zamanı' and no edit form; 'Kaydı düzenle' opens the edit form with 'Değişiklikleri kaydet' for unconfirmed entries; correct-and-confirm planned M4) | /sahip/kayitlar/:id |
...
_Conflict between the approved wireframe (value kept unchanged) and the code written for this task (T4.1/T4.2), which replaced the earlier T3.6 difference. In owner mode /sahip/kayitlar/:id now renders OwnerEntrySummary ('<person> · <plate>', '<date> · <time range>' via formatWorkTimeRange, status 'Henüz doğrulanmadı' / 'Teslim doğrulandı' / 'Onay gerekmiyor', rows Hasılat, Mazot, Diğer masraf (only when used), Şoför payı, 'Beklenen teslim' or 'Giderlerden sonra kalan'; no version line), then WorkEntryConfirmPanel ('Aldığım tutar (TL)' prefilled with the expected amount, 'Beklenenden <amount> az./fazla.', 'Parayı aldım, tutar doğru'; after confirmation 'Alınan tutar' and 'Doğrulama zamanı'), then 'Kaydı düzenle' as a button that opens the edit form. Remaining differences from the approved wireframe: the back link is WORK_ENTRY_MESSAGES.backToOwner '← Özet' instead of '< Özete dön', and the page has an h1 'Kayıt detayı' above the summary; the staff page /yonetim/araclar/:id/kayitlar/:entryId still renders the T3.6 'Güncel kayıt' row list with 'Sürüm N' and no confirmation panel, although the approved flow lets staff confirm on behalf of the owner (the API accepts it). The post-confirmation 'Onaylanmış kaydı düzelt' / 'Düzelt ve onayla' state is not implemented yet (correct-and-confirm planned M4); after a first confirmation the code shows no edit control, so its absence is not a conflict. Update the wireframe to the implemented header and staff view, or keep it and change the code?_
