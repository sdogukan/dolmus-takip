...
_Conflict between the approved wireframe (value kept unchanged) and the code written for T4.1/T4.2 and this task (T4.3). In owner mode /sahip/kayitlar/:id renders OwnerEntrySummary ('<person> · <plate>', '<date> · <time range>' via formatWorkTimeRange, status 'Henüz doğrulanmadı' / 'Teslim doğrulandı' / 'Onay gerekmiyor', rows Hasılat, Mazot, Diğer masraf (only when used), Şoför payı, 'Beklenen teslim' or 'Giderlerden sonra kalan'; no version line), then WorkEntryConfirmPanel ('Aldığım tutar (TL)' prefilled with the expected amount, 'Beklenenden <amount> az./fazla.', 'Parayı aldım, tutar doğru'; after confirmation 'Alınan tutar' and 'Doğrulama zamanı'). T4.3 implemented the approved post-confirmation state: for a confirmed driver-kind entry WorkEntryCorrectForm shows a 'Kaydı düzenle' button that opens the form titled 'Onaylanmış kaydı düzelt' (WORK_ENTRY_MESSAGES.correctTitle) with the editable daily fields (date, person, start/end, next-day box, gross, fuel, other expense + note), a live summary with 'Şoför payı (%20)' and 'Yeni beklenen teslim', 'Aldığım tutar (TL)' prefilled with the confirmed received amount plus the difference note, 'Düzelt ve onayla' and 'Vazgeç'. Remaining differences from the approved wireframe: the correction form has no 'Geçmişi gör' link (GET /work-entries/:id/history is still planned) and no '<person> · <plate>' line of its own (the owner summary above it carries one); the back link is WORK_ENTRY_MESSAGES.backToOwner '← Özet' instead of '< Özete dön', and the page has an h1 'Kayıt detayı' above the summary; the staff page /yonetim/araclar/:id/kayitlar/:entryId still renders the T3.6 'Güncel kayıt' row list with 'Sürüm N' and neither the confirmation panel nor the correction form (tests/e2e/work-entry-edit.spec.ts asserts no correction control there), although the approved flow lets staff confirm and correct on behalf of the owner and the API accepts both. Update the wireframe to the implemented header, correction form and staff view, or keep it and change the code?_
...
_Conflict between the approved notes (value kept unchanged) and the code written for this task (T4.1/T4.2). Approved: 'After confirmation: "Teslim doğrulandı", received amount, time and actor (staff shown as "Sahip adına platform desteği · <ad>")'. Code: WorkEntryConfirmationView in src/server/usecases/work-entries/queries.ts carries only receivedCents, confirmedAt and entryVersion and is documented as carrying no actor or session identity; WorkEntryConfirmPanel shows the status 'Teslim doğrulandı', 'Alınan tutar' and 'Doğrulama zamanı' only. The actor is stored on cash_confirmations and, for staff, in admin_audit work_entry.confirm. The other approved notes match the code: the received field is prefilled with the expected amount and the prefill is not a confirmation; received ≥ 0 and explicit; the driver cannot confirm or edit a confirmed entry; the 409 conflict text is the canonical F7 text. Correct the notes, or keep them and return and show the actor?_
