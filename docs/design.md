...
_Conflict between the approved notes (value kept unchanged) and the code. Now consistent after this task (S3.6, T3.7 in code): the post-save result in WorkEntryForm has a 'Yenile' button that re-reads the entry through GET /api/v1/work-entries/:id, with no background polling; while a result is unknown the form stays in place, so 'Başka bir çalışma kaydı gir' is not shown (asserted in driver-daily-form.spec.ts and owner-staff-work-entry.spec.ts); pending shows 'Henüz doğrulanmadı'; no 'teslim ettim' action is offered. Still open: (1) naming — approved 'No "Kayıtlarım" privacy promise' (DESIGN §2.3: 'Kayıtlarım adıyla bireysel gizlilik güvencesi verilmez.'), code WORK_ENTRY_MESSAGES.listTitle and listLink 'Kayıtlarım' and the /sofor/kayitlar page title 'Kayıtlarım — Dolmuş Takip', followed by the 'Kimin kayıtları?' person picker; (2) confirmed state — approved 'Teslim doğrulandı' + received amount + confirmation time, code WORK_ENTRY_MESSAGES.statusConfirmed 'Onaylandı' on the post-save result (after 'Yenile'), in DriverEntriesList and in WorkEntryEditForm, with no received amount or time; (3) 'Yenile' on the other screens — WorkEntryEditForm re-reads only after a 409 and DriverEntriesList loads on open and on person change; neither has a 'Yenile' control. Rename the list, use 'Teslim doğrulandı' and add 'Yenile' to the detail page, or accept the implemented texts and drop the rule from the notes?_
...
_Conflict between the approved wireframe (value kept unchanged) and the code written for T4.1/T4.2, T4.3 and this task (work-entry history). The approved wireframe places 'Geçmişi gör' in the post-confirmation correction state, next to 'Vazgeç'. This task added a 'Geçmişi gör' link (WORK_ENTRY_MESSAGES.historyLink) at the bottom of /sahip/kayitlar/:id, below the owner summary, the confirmation panel and the correction form, visible in every state (pending, confirmed, owner kind); it opens a separate read-only page /sahip/kayitlar/:id/gecmis ('← Kayıt detayı', h1 'Kayıt geçmişi', the 'Güncel kayıt' summary and the 'Değişiklikler' list), which has no approved wireframe of its own. The staff detail page /yonetim/araclar/:id/kayitlar/:entryId gained the same link to /yonetim/araclar/:id/kayitlar/:entryId/gecmis; the driver detail page has none (tests/e2e/work-entry-edit.spec.ts asserts it). Earlier differences remain: in owner mode the page renders OwnerEntrySummary ('<person> · <plate>', '<date> · <time range>', status, amount rows ending with 'Beklenen teslim' or 'Giderlerden sonra kalan'; no version line) and WorkEntryConfirmPanel; the correction form 'Onaylanmış kaydı düzelt' (WorkEntryCorrectForm) has no '<person> · <plate>' line of its own; the back link is WORK_ENTRY_MESSAGES.backToOwner '← Özet' instead of '< Özete dön' and the page has an h1 'Kayıt detayı'; the staff page still renders the T3.6 'Güncel kayıt' row list with 'Sürüm N' and neither the confirmation panel nor the correction form, although the approved flow lets staff confirm and correct on behalf of the owner and the API accepts both. Update the wireframe to the implemented link placement, header, correction form and staff view and add a history-page wireframe, or keep it and change the code?_
...
_Conflict between the approved notes (value kept unchanged) and the code written for T4.1/T4.2 and this task (work-entry history). Approved: 'After confirmation: "Teslim doğrulandı", received amount, time and actor (staff shown as "Sahip adına platform desteği · <ad>")'. Detail page: WorkEntryConfirmationView in src/server/usecases/work-entries/queries.ts still carries only receivedCents, confirmedAt and entryVersion, and WorkEntryConfirmPanel shows 'Teslim doğrulandı', 'Alınan tutar' and 'Doğrulama zamanı' only. History page (this task): GET /api/v1/work-entries/:id/history returns every cash_confirmations row with its actor, and WorkEntryHistory renders it as 'Teslim doğrulandı', 'Sürüm N · <time>', formatHistoryActor ('<username> · Yönetici|Destek' for staff, 'Sahip oturumu · <plate>' for the owner session) and, for staff only, formatHistoryOnBehalf ('Sahip adına <owner name>'), plus 'Alınan tutar: <amount>' and a 'Güncel' badge on the current version's confirmation. The actor is therefore shown, but on a separate page and not in the approved 'Sahip adına platform desteği · <ad>' wording. The other approved notes match the code: the received field is prefilled with the expected amount and the prefill is not a confirmation; received ≥ 0 and explicit; the driver cannot confirm or edit a confirmed entry; the 409 conflict text is the canonical F7 text. Correct the notes to the detail + history split, or keep them and show the actor on the detail page in the approved wording?_
