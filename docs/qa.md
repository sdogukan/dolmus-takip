...
_One row per architecture flow (7 flows ↔ 7 scenarios). Currently 5 E2E spec files exist (vehicle-login, platform-login, admin-businesses, admin-vehicles, login-accessibility)._
...
_Existing test files enumerated from src and tests/; planned targets from QA-PLAN critical details. T2.2 added src/server/auth/vehicle-password.test.ts, src/lib/draft-version.test.ts, tests/integration/admin-vehicles-routes.test.ts and race cases in session-usecases / vehicle-login-route tests._
