 ...
 *
 * K9 — drizzle-kit 0.31.10. Görev tanımı: "dialect sqlite, out ./drizzle"
 * — `out` migration SQL dosyalarının commit'lenebilir konumu, veri şemasını
 * üreten kaynak `schema` bu paketin tek dosyası `src/server/data/schema.ts`'tir.
 *
 ...
 * migrator`'ın `migrate()` fonksiyonuyla uygulanır; ilk şema kurulumuna
 * yalnız açık migration/kurulum komutu izin verir). Bu yüzden burada gerçek
 * bir DB dosyasına ihtiyaç yoktur; yol,
