/**
 * GET /api/v1/reports/people — kişi bazlı dönem raporu (person_id başına bir satır).
 *
 * `period` / `date` ayrıştırması `/reports/vehicles` ile aynıdır; yalnız URL'den
 * okunur, bilinmeyen parametre yok sayılır. İşletme/araç yalnız oturum
 * kapsamından gelir; şoförde `report.read` yoktur (403).
 */
import { z } from "zod";
...
import { readPeoplePeriodReportForScope } from "../../../../../server/usecases/reports";
...
export const GET = withProtectedRoute({
  ...
  if (!ctx.scope) {
    throw new Error("GET /reports/people: scope eksik (programlama hatası).");
  }
  ...
  if (!period) throw new Error("GET /reports/people: doğrulanmış tarih çözülemedi (programlama hatası).");

  const report = readPeoplePeriodReportForScope(ctx.db, ctx.scope, period);
